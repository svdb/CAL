...
if (!empty($_REQUEST['id']))
{
  ...
  else
  {
    ...
    $on_home_page = (boolean) $post->on_home_page;
...
if (!empty($_POST) && $can_edit_post)
{
  ...
  $on_home_page = !empty($_POST['on_home_page']);
...
if (!empty($_POST) && !empty($_POST['save']) && $can_edit_post)
{
  ...
  $cur->on_home_page = (integer) $on_home_page;
  $cur->is_new = (integer) $is_new;
...
if ($can_edit_post)
{
  ...
  '<p><label for="on_home_page" class="classic">'.form::checkbox('on_home_page',1,$on_home_page).' On Home Page</label></p>'.
  '<p><label for="is_new" class="classic">'.form::checkbox('is_new',1,$is_new).' New!</label></p>'.
  #  '<p><label for="is_new" class="classic"><input type="checkbox" name="is_new" value="'.$is_new.'" id="is_new" onclick="checkHome(this)"> New!</label></p>'.
...
echo '<script type="text/javascript">'.
       '$("#is_new").click(function() {'.
         'if ($(this).is(":checked")) {'.
           '$("#on_home_page").attr("checked","checked");'.
         '}'.
       '});'.
     '</script>';
