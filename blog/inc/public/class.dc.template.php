...
class dcTemplate extends template
{
	...
	*/
	public function Entries($attr,$content)
	{
		...
		if (isset($attr['on_home_page'])) {
			$p .= "\$params['on_home_page'] = ".(integer) (boolean) $attr['on_home_page'].";";
		}

		if (isset($attr['is_new'])) {
			$p .= "\$params['is_new'] = ".(integer) (boolean) $attr['is_new'].";";
		}
	...
	*/
	...
	on_home_page	(0|1)	#IMPLIED	-- post is on home page (value : 1) or not (value : 0)
	is_new	(0|1)	#IMPLIED	-- post is marked as new (value : 1) or not (value : 0)
	...
	*/
	public function EntryIf($attr,$content)
	{
		...
		if (isset($attr['on_home_page'])) {
			$sign = (boolean) $attr['on_home_page'] ? '' : '!';
			$if[] = $sign.'(boolean)$_ctx->posts->on_home_page';
		}

		if (isset($attr['is_new'])) {
			$sign = (boolean) $attr['is_new'] ? '' : '!';
			$if[] = $sign.'(boolean)$_ctx->posts->is_new';
		}
	...
	*/
	...
	{
		return
		'<?php printf(__("Powered by %s"),"<a target=\"_blank\" href=\"http://dotclear.org/\">Dotclear</a>"); ?>';
	}
